...
namespace RadixDlt.CoreApiSdk.Model
{
    ...
    public partial class Validator : IEquatable<Validator>, IValidatableObject
    {
        ...
        /// Initializes a new instance of the <see cref="Validator" /> class.
        /// </summary>
        /// <param name="validatorAddress">validatorAddress (required).</param>
        /// <param name="stake">stake (required).</param>
        public Validator(string validatorAddress = default(string), string stake = default(string))
        {
            // to ensure "validatorAddress" is required (not null)
            if (validatorAddress == null)
            {
                throw new ArgumentNullException("validatorAddress is a required property for Validator and cannot be null");
            }
            this.ValidatorAddress = validatorAddress;
            // to ensure "stake" is required (not null)
            if (stake == null)
            {
                throw new ArgumentNullException("stake is a required property for Validator and cannot be null");
            }
            this.Stake = stake;
        }
        ...
        {
            // Stake (string) pattern
            Regex regexStake = new Regex(@"^-?[0-9]+$", RegexOptions.CultureInvariant);
            if (false == regexStake.Match(this.Stake).Success)
            {
                yield return new System.ComponentModel.DataAnnotations.ValidationResult("Invalid value for Stake, must match a pattern of " + regexStake, new [] { "Stake" });
            }
