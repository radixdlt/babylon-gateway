...
internal class MetadataProcessor
{
    ...
    private MetadataProcessor(List<MetadataChangePointer> metadataChangePointers, SequencesHolder sequences)
    {
        _metadataChangePointers = metadataChangePointers;
        _sequences = sequences;
    }

    public static async Task<MetadataProcessor> Create(List<MetadataChangePointer> metadataChangePointers, ReadHelper readHelper, SequencesHolder sequences, CancellationToken token)
    {
        return new MetadataProcessor(metadataChangePointers, sequences)
        {
    ...
    public void DoSth()
    {
        foreach (var e in _metadataChangePointers)
        {
            ...
            if (!MostRecentAggregatedMetadataHistory.TryGetValue(e.ReferencedEntity.DatabaseId, out var previousAggregate) || previousAggregate.FromStateVersion != e.StateVersion)
            {
                ...
                MostRecentAggregatedMetadataHistory[e.ReferencedEntity.DatabaseId] = aggregate;
            }
            else
            {
