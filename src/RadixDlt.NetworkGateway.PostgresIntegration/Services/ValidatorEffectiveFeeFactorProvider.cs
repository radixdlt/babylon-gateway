...
public static class ValidatorEffectiveFeeFactorProvider
{
    public static GatewayModel.ValidatorCollectionItemEffectiveFeeFactor ExtractFeeFactorFromValidatorState(string validatorState, long currentEpoch)
    {
        var parsedState = JsonConvert.DeserializeObject<CoreModel.ValidatorFieldStateValue>(validatorState);

        if (parsedState == null)
        {
            throw new UnreachableException($"Failed to parse state for validator. State: {validatorState}");
        }
