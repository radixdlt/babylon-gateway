...
WHERE d.key_value_store_entity_id = @keyValueStoreEntityId AND (d.from_state_version, d.id) >= (@cursorStateVersion, @cursorId) AND d.from_state_version <= @stateVersion
ORDER BY d.from_state_version, d.id
LIMIT @limit
;",
            parameters: new
            {
                ...
                cursorStateVersion = cursor?.StateVersionBoundary ?? 0,
                cursorId = cursor?.IdBoundary ?? 0,
                limit = pageSize + 1,
            },
