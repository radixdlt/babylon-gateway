...
export interface StateEntityDetailsResponseComponentDetailsAllOf {
    ...
    /**
     * Component inner state representation. Depending on entity type this field might be either a programmatic JSON
     * structure (see `ProgrammaticScryptoSborValue`) or custom JSON model defined in Core API. Former is used only for
     * `GenericComponent` while latter is used for all "native" components such as `Account`, `Validator`,
